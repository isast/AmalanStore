...
if(isset($_REQUEST['submit']))
{
	...
	if( empty($_REQUEST['name']))
	{
		...
		$iname = $_REQUEST['name'];
		mysqli_query($con, "UPDATE newArrivals SET name='$iname' WHERE id='$id'");
	...
	if ($_FILES)
	{
			...
			&& in_array($extension, $allowedExts))
		{
			...
			elseif (file_exists("upload/" . $_FILES["file"]["name"]))
			{
				...
				$query = mysqli_query($con,"SELECT filename FROM newArrivals WHERE id='$id'");
				...
				mysqli_query($con,"UPDATE newArrivals SET filename = '$file' WHERE id='$id'");
				header("Location:adminhome.php");
			}
	}
}
